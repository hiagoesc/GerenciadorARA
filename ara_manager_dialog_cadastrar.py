...
from qgis.core import QgsProject, QgsFeature, QgsGeometry, QgsPointXY, QgsExpression, QgsFeatureRequest, QgsCoordinateReferenceSystem, QgsCoordinateTransform
...
class CadastroARADialog(QtWidgets.QDialog, Ui_GerenciadorARADialogCadastrar):
    ...
    def criar_ponto_transformado(lat, lon, epsg_origem, epsg_destino):
        # 1. Cria ponto em WGS84 (EPSG:4326)
        src_crs = QgsCoordinateReferenceSystem(epsg_origem)
        dest_crs = QgsCoordinateReferenceSystem(epsg_destino)
        transform = QgsCoordinateTransform(src_crs, dest_crs, QgsProject.instance())

        ponto_wgs84 = QgsPointXY(float(lon), float(lat))  # atenção: ordem é (x=lon, y=lat)
        
        # 2. Transforma para o SRC destino
        return transform.transform(ponto_wgs84)
    ...
    def _obter_ponto_processo(self, inscricao):
        """
        Retorna:
            ponto_dest: QgsPointXY no CRS da camada 'processos_ara'
            lat_wgs, lon_wgs: floats em WGS84 (EPSG:4326) para usar nos links
        """
        # CRS de destino = CRS da camada onde você grava o ponto
        camada_proc = QgsProject.instance().mapLayersByName("processos_ara")[0]
        crs_dest = camada_proc.crs()

        lotes_list = QgsProject.instance().mapLayersByName("LOTES")
        if lotes_list:
            camada_lotes = lotes_list[0]
            crs_lotes = camada_lotes.crs()

            def _centroide_por_rotulo(rotulo):
                camada_lotes.selectByExpression(f""""rotulo" = '{rotulo}'""")
                selecionados = list(camada_lotes.selectedFeatures())
                camada_lotes.removeSelection()
                return selecionados[0].geometry().centroid().asPoint() if selecionados else None

            # tenta inscrição completa, depois “/0000”
            p_src = _centroide_por_rotulo(inscricao) or _centroide_por_rotulo(inscricao[:10] + "0000")
            if p_src:
                # lat/lon em WGS84 a partir do CRS dos lotes
                to_wgs = QgsCoordinateTransform(crs_lotes, QgsCoordinateReferenceSystem("EPSG:4326"),
                                                QgsProject.instance())
                p_wgs = to_wgs.transform(p_src)
                lat_wgs, lon_wgs = p_wgs.y(), p_wgs.x()

                # ponto no CRS da camada de processos (se diferente do dos lotes)
                if crs_lotes.authid() != crs_dest.authid():
                    to_dest = QgsCoordinateTransform(crs_lotes, crs_dest, QgsProject.instance())
                    p_dest = to_dest.transform(p_src)
                else:
                    p_dest = p_src

                return p_dest, lat_wgs, lon_wgs

        # ---- fallback: usa lat/lon digitados (já em WGS84) ----
        ...
        to_dest = QgsCoordinateTransform(QgsCoordinateReferenceSystem("EPSG:4326"),
                                        crs_dest, QgsProject.instance())
        p_dest = to_dest.transform(QgsPointXY(lon, lat))
        return p_dest, lat, lon
    ...
    def salvar_dados_processo(self):
        ...
        num_predial = self.lineEditNumPredial.text().strip()
        if not num_predial:
            num_predial = None
        ...
        lat_min = self.lineEditLatitudeMinutos.text().strip()
        lat_seg = self.lineEditLatitudeSegundos.text().strip()
        lon_min = self.lineEditLongitudeMinutos.text().strip()
        lon_seg = self.lineEditLongitudeSegundos.text().strip()
        if lat_min and lat_seg and lon_min and lon_seg:
            lat_dms = f'{self.lineEditLatitudeGraus.text()}°{self.lineEditLatitudeMinutos.text()}{r"'"}{self.lineEditLatitudeSegundos.text()},{self.lineEditLatitudeSegundosDecimais.text()}{r'"'}S' # coordenada Y
            lon_dms = f'{self.lineEditLongitudeGraus.text()}°{self.lineEditLongitudeMinutos.text()}{r"'"}{self.lineEditLongitudeSegundos.text()},{self.lineEditLongitudeSegundosDecimais.text()}{r'"'}O' # coordenada X
        else:
            lat_dms = None
            lon_dms = None

        ponto_dados = self._obter_ponto_processo(inscricao)
        if not ponto_dados:
            QMessageBox.critical(self, "Erro", "Não foi possível obter coordenadas.")
            return

        ponto, lat, lon = ponto_dados  # lat/lon já garantidos em WGS84
